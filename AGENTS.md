...
Packages with **required peer dependencies** get peer-specialized shared entries. A shared global-store entry has nothing above it in the module tree, so Node's upward resolution walk for a peer (`react-dom` → `react`) would dead-end in `<data_dir>/virtual-store`. When every required peer of a package resolves to a single graph version, that peer is symlinked *inside* the entry's own `node_modules` and folded into the entry hash — so `react-dom + react@18` and `react-dom + react@17` get distinct shared entries, and the package can be shared across projects while staying runnable. When a required peer is missing or ambiguous (two installed versions both satisfy the range), the package falls back to project-local, where the peer resolves through the project root. Locality propagates to dependents (a package depending on — or peer-resolving to — a project-local package is itself project-local) and is folded into the install layout hash, so a node_modules produced by an older build self-heals on the next `snpm install`.
