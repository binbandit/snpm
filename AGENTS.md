...
Packages are materialized project-locally (in `<root>/.snpm`) instead of in the shared global virtual store when they are not global-store-safe: patched packages, packages whose lifecycle scripts are allowlisted, and `file:`-sourced packages.

Packages with **required peer dependencies** get peer-specialized shared entries. A shared global-store entry has nothing above it in the module tree, so Node's upward resolution walk for a peer (`react-dom` → `react`) would dead-end in `<data_dir>/virtual-store`. Each required peer is resolved to a concrete version — the root-declared version if it satisfies the range, otherwise the highest satisfying installed version (the same version the hoisted layout would bind) — then symlinked *inside* the entry's own `node_modules` and folded into the entry hash. So `react-dom + react@18` and `react-dom + react@17` get distinct shared entries, and the package can be shared across projects while staying runnable. Only a genuinely missing peer forces project-local materialization. (Fully distinct per-consumer peer bindings would require node duplication and are a separate change.) Locality propagates to dependents (a package depending on — or peer-resolving to — a project-local package is itself project-local) and is folded into the install layout hash, so a node_modules produced by an older build self-heals on the next `snpm install`.
