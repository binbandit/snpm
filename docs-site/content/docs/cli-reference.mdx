...
The generated file includes a normalized package name, `1.0.0` version, `packageManager` pinned to the running `snpm` version, and well-known package.json field ordering.
...
| `SNPM_DISABLE_GLOBAL_VIRTUAL_STORE_FOR_PACKAGES` | Comma-separated packages to force into the project-local virtual store |
