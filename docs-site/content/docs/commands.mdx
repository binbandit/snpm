...
Creates a clean `package.json` in the current directory with a normalized package name, `1.0.0` version, a `packageManager` pin for the current `snpm` version, and well-known package.json field ordering.
...
This prints cache/data locations, registry/auth info, install settings (hoisting, link backend, local virtual-store package fallback, strict peers, frozen lockfile default, min package age, registry concurrency), script allowlist, and logging status.
